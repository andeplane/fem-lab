...
fn a_static_nonlinear_step_reaches_the_elastica_and_keeps_its_load_deflection_curve() {
    ...
    let QueryResult::Result(r) = e.query(Query::Result { step: Some("large".into()) }).unwrap() else { panic!() };
...
fn a_nonlinear_step_that_diverges_names_the_increment_and_the_way_out() {
    ...
    // and the controls it cannot run with are schema errors naming their own field
    for (fields, field) in [
        (r#""increments":0"#, "increments"),
        (r#""nonlinearMaxIterations":0"#, "nonlinearMaxIterations"),
        (r#""nonlinearTolerance":0"#, "nonlinearTolerance"),
        (r#""maxCutbacks":21"#, "maxCutbacks"),
        (r#""tEnd":"0 s""#, "tEnd"),
    ] {
        ok(
            &mut e,
        ...
        let bad = err(&mut e, r#"{"cmd":"solve.run","step":"bad"}"#);
