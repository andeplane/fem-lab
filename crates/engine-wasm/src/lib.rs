...
impl Engine {
    ...
    pub fn new(threads: u32) -> Engine {
        Engine { inner: femlab_engine::Engine::new(Box::new(JsHost), threads.max(1) as usize) }
    }
    ...
    pub async fn dispatch(&mut self, cmd_json: String, on_progress: Option<js_sys::Function>) -> Result<String, JsValue> {
        ...
        let mut cb = |p: Progress| -> bool {
            match &on_progress {
                Some(f) => {
                    let r = f.call3(&JsValue::NULL, &JsValue::from_str(p.phase), &JsValue::from_f64(p.fraction), &JsValue::from_str(&p.message));
    ...
    pub async fn replay_hashes(&mut self, journal_json: String, skip_solves: bool, verify: bool) -> Result<String, JsValue> {
