...
export function sessionEngine(wasm: CheckedModule, threads: number, epoch: string): EngineHandle {
  ...
  const acquire = () => ordered(() => {
    const current = JSON.parse(owner.stamp()) as Stamp;
    let lease = JSON.parse(owner.begin_run(JSON.stringify(current.session))) as RunLease;
    let operation = 0n;
    let revoked = false;
    const context = () => {
  ...
  const unscoped = (): never => { throw new FemError('session.expired', 'acquire a request lease before calling the engine'); };
  return { acquire, dispatch: unscoped, query: unscoped, modelFile: unscoped };
}
