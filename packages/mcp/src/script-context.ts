// QuickJS is the capability boundary; no Node object or function enters its heap.
...
// Guest JavaScript is data to V8; exercise it through the real QuickJS runtime in tests.
...
export async function createScriptContext(code: string, send: (text: string) => void) {
  ...
  const emit = vm.newFunction('__send', (value) => {
    ...
    if (text.length > SCRIPT_MESSAGE_BYTES) throw new Error('script message exceeds 1 MiB');
    send(text);
  });
