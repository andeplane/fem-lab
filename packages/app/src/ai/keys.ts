...
// person typed, kept in this tab's sessionStorage, else the shell key `vite.config.ts` injects
...
export { KEY_SLOT, storeKey, migratePersistentKeys } from './key-storage';
...
export function resolveKey(id: ProviderId, storage: Storage | null = browserStorage('sessionStorage'), dev = devApiKeys()): KeyInfo {
...
export function defaultProvider(storage: Storage | null = browserStorage('localStorage'), dev = devApiKeys(), keys: Storage | null = browserStorage('sessionStorage')): ProviderId {
