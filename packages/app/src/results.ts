...
import type { Field, ResultSummary, StudyReport, Warning } from '@femlab/registry';
import { FIELD_CHOICES, choiceOf, displayUnitOf, siUnitOf } from './fields';
...
export class ResultsView {
  ...
  private async factor(field: Field): Promise<number> {
  ...
  async refresh(force = false): Promise<void> {
    ...
    const key = `${result.step}|${this.store.state.fieldKey}|${String(this.store.state.clamp)}`;
  ...
  private async load(result: ResultSummary): Promise<void> {
    ...
    const [scalar, factor] = await Promise.all([
      this.transport.field(result.step, choice.field, choice.component ?? undefined),
      this.factor(choice.field),
    ]);
    const values = magnitude(scalar.values, choice.magnitude === true);
    ...
    const clamp = this.store.state.clamp;
    const range: [number, number] = clamp ?? [min, max];
    v.setField(values, range);
    this.store.set({ legend: { min: range[0], max: range[1], unit: displayUnitOf(choice.field, this.store.state.model?.units) } });

    const u = await this.transport.field(result.step, 'displacement');
    this.store.set({ lengthFactor: await this.factor('displacement') });
    this.displacement = u.values;
