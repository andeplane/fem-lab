...
describe('tutorials/*.json', () => {
  ...
  it('bundles the four built-ins the PLAN promises plus one per procedure', () => {
    ...
    expect(TUTORIALS.map((t) => t.id).sort()).toEqual(
      ['cantilever', 'heat-conduction', 'journal-as-program', 'mesh-convergence', 'modal-analysis', 'plate-with-hole', 'read-a-result', 'solve-cost-and-solvers', 'symmetry-and-2d', 'thermal-bar', 'transient-heat'].sort(),
    );
