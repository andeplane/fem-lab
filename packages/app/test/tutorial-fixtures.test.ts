...
describe('tutorials/*.json', () => {
  ...
  it('bundles the four built-ins the PLAN promises plus one per procedure', () => {
    ...
    expect(TUTORIALS.map((t) => t.id).sort()).toEqual(
      ['cantilever', 'heat-conduction', 'mesh-convergence', 'modal-analysis', 'plate-with-hole', 'read-a-result', 'symmetry-and-2d', 'thermal-bar', 'thermal-stress-chaining', 'transient-heat'].sort(),
    );
