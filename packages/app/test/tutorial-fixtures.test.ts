...
describe('tutorials/*.json', () => {
  ...
  it('bundles the four built-ins the PLAN promises plus one per procedure', () => {
    ...
    expect(TUTORIALS.map((t) => t.id).sort()).toEqual(
      ['cantilever', 'heat-conduction', 'journal-as-program', 'mesh-convergence', 'modal-analysis', 'plate-with-hole', 'pressure-vessel', 'free-free-modal', 'read-a-result', 'symmetry-and-2d', 'thermal-bar', 'transient-heat', 'composite-block-shear', 'solve-cost-and-solvers', 'thermal-stress-chaining', 'steel-roof-truss'].sort(),
    );
