...
describe('fieldsOf', () => {
  ...
  it('reads an enum as options and a list of names as a multi picker', () => {
    ...
    expect(fields.find((f) => f.path[0] === 'procedure')).toMatchObject({ kind: 'enum', options: ['static', 'static-nonlinear', 'modal', 'heat-steady', 'heat-transient', 'explicit', 'implicit'], multi: false });
