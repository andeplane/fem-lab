...
export const HOST_COMMANDS: HostDef[] = [
  ...
  def('file.autosave', 'Turn background autosave on or off. When on (the default), Command changes are debounced into a bounded history in this browser\'s IndexedDB; file.restore reopens a revision. A pending or quota-failed revision is restorable in this tab but is not durable until its write commits. Nothing is uploaded. Turning autosave off forgets prior revisions after any in-flight write finishes; revisions created after turning it back on are retained.', z.object({ on: z.boolean() }), ({ on }, ctx) => {
    ctx.files.setAutosave(on);
    return ctx.files.autosave();
  }),
  def('file.restore', 'Reopen an autosaved Journal revision onto the current Model. Pass the `id` from query.autosaveHistory to reopen an earlier revision; omit it for the newest. Returns `{ name, at, commands }`, or `null` when this browser has nothing saved.', z.object({ id: z.string().optional() }), ({ id }, ctx) => ctx.files.restore(id)),
...
export const HOST_QUERIES: HostDef[] = [
  ...
  def('query.autosaveHistory', 'List up to 20 locally known Journal revisions, newest first, including pending writes and retryable storage failures. Each `id` stays restorable through file.restore until history eviction or autosave clearing, including when model names and times tie. Pending revisions are not durable until their write commits.', none, (_, ctx) => ({ enabled: ctx.files.autosave().enabled, revisions: ctx.files.autosaves() })),
];
