...
import { z } from 'zod';
import type { ModelFile } from './generated/engine';
...
export const LegendSpec = z.object({
  colormap: z.enum(['viridis', 'rainbow']).optional(),
...
export interface HostContext {
  ...
  view: {
    ...
    showField(f: z.output<typeof FieldChoice>): void;
    setLegend(l: z.output<typeof LegendSpec>): void;
    setDeformScale(s: z.output<typeof DeformScale>): void;
...
export const HOST_COMMANDS: HostDef[] = [
  ...
  def('file.export', 'Export in any format the engine offers (see query.exportFormats): mesh, results, script, Journal or report. Lands in the project folder when one is open (or `to: "project"`), else downloads.', z.object({ spec: z.looseObject({ format: z.string() }), name: z.string().optional(), to: Destination }), async ({ spec, name, to }, ctx) => {
    const out = await ctx.transport.export(spec as ExportSpec);
    return deliver(ctx, to, name ?? out.filename, out.mime, out.bytes);
  }),
