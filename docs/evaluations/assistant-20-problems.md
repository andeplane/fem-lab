...
3. `query.result.balance <= 1e-9`. This checks reaction balance independently of the requested value,
   including the zero-total modal and free-fall cases.
...
The prompts will include the geometry, boundary conditions, requested output and the instruction
"build this from scratch, solve it, check reaction balance, and compare the answer with a hand
calculation." They do not include the reference answer. Lengths below use x as the long direction;
...
| H1 | Steady heat bar: L 0.20 m, ends 0 K and 100 K, constant k 40 W/(m K) | T(L/2) = 50.0 K | 1e-8 K | linear conduction |
| H2 | Steady heat bar: L 0.50 m, ends 300 K and 360 K, constant k 16 W/(m K) | T(L/2) = 330.0 K | 1e-8 K | linear conduction |
| H3 | Steady heat bar: L 0.12 m, ends 273.15 K and 373.15 K, constant k 205 W/(m K) | T(L/2) = 323.15 K | 1e-8 K | linear conduction |
| H4 | Steady heat bar: L 1.0 m, ends 320 K and 280 K, constant k 1.4 W/(m K) | T(L/2) = 300.0 K | 1e-8 K | linear conduction |
...
| D1 | Unconstrained 100 mm cube in −z gravity 9.81 m/s², explicit to 1 ms | uz = gt²/2 = −0.004905 mm | 0.5% | final retained frame |
| D2 | Unconstrained 80 mm cube in +y gravity 3.711 m/s², explicit to 2.5 ms | uy = gt²/2 = +0.0115969 mm | 0.5% | final retained frame |
...
| V1 | Axial bar: L 0.9 m, 30 × 20 mm, E 90 GPa, +12 kN. Repair supplied script whose box length is typed as seconds. | ux = +0.200 mm | 2% | invalid validation, no mutation, corrected validation |
...
cases; secrets are absent from serialized artifacts; and unavailable credentials produce the
