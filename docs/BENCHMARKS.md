...
**F12 is gated at roundoff on purpose.** A single degree of freedom is the one case where mode
...
**F13's amplitude gate is 4 %, not 3 %, and the reason is physics rather than slack.** The
...
**F14 and F15 are not claimed.** NAFEMS R0016, *Selected Benchmarks for Forced Vibration*, is the
...
number, so the rows say **resolve** and the harmonic PR is gated on F12 and F13, which are closed
forms.
