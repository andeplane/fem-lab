...
| 21 | B's seam expects `cost_estimate(mesh, opts) -> CostEstimate` from A; A never defines it | Small gap, but a coder would stall on it | `query.cost` lands in commit 19; amended by #122 to bounded counting and explicit mandatory-storage lower bounds with unknown feasibility below the planning budget (A §5) |
